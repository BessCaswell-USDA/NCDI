...
//	*Description: Disaggregates the FNDDS 4.1 food list over multiple iterations
...
//	*1. Match food codes to ingredients
		{
		//* Iteration 1
			//*	Call in the FNDDS4.1 food list. This dataset was initially imported as an SAS dataset and converted to Stata.
				use mainfooddesc
		
			//*	Drop all ingredient metadata the WWEIA information and nutrient values. Nutrient values will be connected later after disaggregation.
			...
			//*	Merge the FNDDS 4.1 food composition table with the FNDDS ingredient list, which was also initially imported as an Excel file with the first row used as variable names. We used 1:m merging as each food code is unique, but there are many ingredients that could be linked to a single food code.
		...
		//*	Iteration 3 (steps for Iterations 3 and 4 serve identical purposes to the steps for Iteration 2)
			gen it3 = _n
...
//	*2. Merge the ingredient disaggregated database to the ingredient nutrient values
		...
		//*	Merge ingredients using the joinby command
			joinby ingredientcode using FNDDS4_ingnutrval, unmatched(master) _merge(newv)
		...
		//*	Save
			save "FNDDS4_ingredientizednutrval.dta", replace
		}
